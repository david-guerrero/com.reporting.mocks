...
public class RiskResultPubSubProducer {
    private PubSubProducer producer = null;

    public RiskResultPubSubProducer(ApplicationConfig appConfig) {
        producer = new PubSubProducer(appConfig.getIntradayRiskTickTopic(), appConfig.getProjectId());
    }
    ...
    public void send(RiskResult riskResult) {
        if (this.producer != null) {
            ...
            records.add(gson.toJson(riskResult));
            try {
